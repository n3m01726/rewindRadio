...
<div class="row border-bottom border-3 bg-light p-2">
    ...
    <div class="col-9">
    <div class="title text-uppercase fw-bold"><?= $row['clean_date']; ?> - <a href="<?php $id = $row['id']; echo $router->generate('article', ['id' => $id]);?>"><?= Text::cutText($row['title'], 80) ?></a></div>
        <div class='artist'>  <?= Text::cutText(shortcodes::remove_shortcodes($row['content']),100); ?></div>
            <div class="meta">
                <?= $lang['posted_by']; ?><a href="<?php $id = $row['posted_by']; echo $router->generate('profile', ['id' => $id]);?>">
                <?php if(isset( $row['nice_nickname'])) { echo  $row['nice_nickname'];} else {echo $row['username'];} ?></a>
            </div>
    </div>
</div>
<?php }
...
<div class="card" style="width: 25rem;">
  ...
  <div class="card-body">
    <h5 class="card-title"><?= $row['clean_date']; ?> - <a href="<?php $id = $row['id']; echo $router->generate('article', ['id' => $id]);?>"><?= Text::cutText($row['title'], 80) ?></a></span></h5>
    <p class="card-text"><?= Text::cutText(shortcodes::remove_shortcodes($row['content']),80); ?></p>
  </div>
  <div class="card-footer">
  <?= $lang['posted_by']; ?><a href="<?php $user_id = $row['posted_by']; echo $router->generate('profile', ['id' => $user_id]);?>">
                <?php if(isset( $row['nice_nickname'])) { echo  $row['nice_nickname'];} else {echo $row['username'];} ?></a>
  </div>
</div>
